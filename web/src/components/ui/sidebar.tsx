  ...
  if (isMobile) {
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          {/*
            本地汉化：注册表原文是 "Sidebar" / "Displays the mobile sidebar."。
            这几句是读屏用户听到的层名，全站文案是中文，不该在这里冒英文。
          */}
          <SheetHeader className="sr-only">
            <SheetTitle>侧边导航</SheetTitle>
            <SheetDescription>移动端的侧边导航抽屉。</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      {/* 本地汉化，同上层 SheetTitle */}
      <span className="sr-only">展开或收起侧边导航</span>
    </Button>
  )
}
