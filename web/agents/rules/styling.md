...
- **另一半在 `MemeImage` 的 `<img>` 上**：图到了从 `opacity-0` 抬到 1，同样 200ms，
  与占位块摘掉是同一帧——两条合起来才是完整的那一次「出现」（灰块出现一次、图出现一次）。
  ⚠️ **斜坡那 200ms 底下没有灰色垫底**，露的是图片框自己的白底（`bg-white`，两个主题都是）。
  要留住灰色背景，就得让占位块在 DOM 里多活 200ms 当垫层——那期间那道**无限循环**的扫光
  也得多跑，等于每个已加载的格子都常驻一个节点换一次 200ms 的观感。**这是有意的取舍**，
  改之前先读 `MemeImage.tsx` 里 `<img>` 那段注释。
- **`prefers-reduced-motion` 下扫光停住，且停在容器外**——`animate-none` 之后靠的就是那个
  `left` 静止位；若改用 `translate-x` 摆位，这一档会留下正中一道白条。图那一半同样停
  （`motion-reduce:transition-none`），这条与 `motion-reduce:animate-none` 一样，**靠的是
  「变体规则排在后面」**——顺序一变两条一起失效。

量法是几何与时间的：采样扫光条的 `rect.x`，它必须从容器左外侧走到右外侧；图那半条沿用
**逐帧记账**（`requestAnimationFrame` 里读 `getComputedStyle(img).opacity`），第一帧必须是 0、
中间要有斜坡、要淡到 1（`scripts/verify-shimmer.mjs` 的 `sweep()` 与第 6 节）——
「只有半条」和「一帧跳到位」这两种错，读代码都看不出来。
