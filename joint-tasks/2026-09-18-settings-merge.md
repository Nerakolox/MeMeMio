...
状态：`done`（2026-09-24 总管裁定：两处 SPEC 措辞偏差已结，实质约束未动，见「与 SPEC 的偏差」末节）
开始：2026-09-18
...
**两处都判「改措辞」，实质约束一条不动。** 理由是同一条：**「哪些人看得见什么」是契约，「在哪个 URL 下、分成几块」不是。** 本任务动的恰好只有后者，所以这不是「把 SPEC 改成代码的样子」，而是一句描述过时了。

- [§9.6](../spec/09-decisions.md)：「普通用户的设置页里没有这一组」→ **「普通用户看不到这一组」**，并加了一句注明它现在与视觉模型同处 `/settings`、仍不渲染给普通用户。全站一份 / 仅管理员可改 / 换模型触发全站重算 / 归一化到 1024 四条实质约束**逐条照旧**。
- [§9.9](../spec/09-decisions.md)：「分成两个页面」→ **「分成两类分段」**（视觉模型 / Embedding 与运行参数），并注明两类都在 `/settings` 上按权限分块渲染。
- 两处都带日期留痕，按 [§8.4](../spec/08-collaboration.md) **不静默改写**。
