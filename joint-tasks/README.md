...
| [页面地图：把「动作在哪出现」定成规则](2026-09-25-web-page-map.md) | `planning` | web 单端为主 | **要先裁定才能动手**：四处取舍写在任务文件 §3（发送入口准入 / 管理动作准入 / **单张图要不要有地址** / 管理员入口分层与设置页分区），各带选项与倾向。第 3 处一动就是跨端契约，要 SPEC §8 先行。本次只写文档、不动 `web/src/` |
