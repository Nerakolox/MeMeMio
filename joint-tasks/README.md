...
| [会话与访问收口：读路径登录、限流、注册竞态、401 跳转](2026-09-24-auth-access-hardening.md) | `in_progress` | 跨端 | **只差 §8 那两条联合验收**（401 → 跳登录 → 回原页；429 → 倒计时）：api 已合入（`4f6a97f`）、web 四条已落、两端验收都已回填，但 web 那轮对着替身走的——重跑要盯的三处见任务文件 §9。§4 第 1 项已裁定（[§9.32](../spec/09-decisions.md)），第 2 项（测试连接拒私网）仍待裁定 |
...
| [浏览页图墙：Ctrl + 滚轮换密度](2026-09-26-浏览页捏合缩放.md) | `in_progress` | web 单端 | **实现与本端验收完成、已提交**（headless Chrome 脚本 ALL PASS：锚点纵向 0px、换档不重挂不取数、图墙外不拦截、窄屏不变）。欠账：真触控板 / Firefox / Safari 没测；横向不钉（瀑布流决定落哪列）；只用键盘的人换不了密度 |
| [首次部署与积压的联合验收](2026-09-24-first-deploy.md) | `in_progress` | 总管 + 运维 | 手册 §9「首次部署」已写（命令序列、Caddy / nginx 样例、首个管理员按 [SPEC §9.32](../spec/09-decisions.md) 靠顺序避开），**一条命令都还没真跑过**——本机 Docker 没开，镜像构建也没复跑。下一步本机走一遍再上服务器 |
