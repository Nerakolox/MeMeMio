...
| [首页改成入口页：搜索只做入口，主体是图墙 + 两条 rail](2026-09-26-首页改版.md) | `in_progress` | 跨端，**web 先** | **web 段完工并走查通过**（走查脚本 47 条断言全绿，1264×900 与 390×844 各走一遍，实测与截图见任务文件 §9）。api 段未开工：删 `/search` + 改 SPEC §6.3.3/§1.3 **必须等 web 合入并验收之后再开**，提前删会把首页当场打断——api 交接段总管届时才写。另：§4 要求的两条（图墙只给发送、键盘选图不做）要落进页面地图那张「动作在哪些面出现」的表，而那张表随[页面地图](2026-09-25-web-page-map.md)还停在 `planning` |
