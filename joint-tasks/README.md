...
| 任务 | 状态 | 端 | 现在卡在哪 |
|---|---|---|---|
| [首页改成入口页：搜索只做入口，主体是图墙 + 两条 rail](2026-09-26-首页改版.md) | `planning` | 跨端，**web 先** | 契约已定（产品负责人 2026-09-26 七条），没人动手。web 端零接口改动、无阻塞点；**api 端只是收尾（删 `/search` + SPEC §6.3.3/§1.3），必须等 web 合入并验收之后再开**，提前删会把首页当场打断——api 交接段总管届时才写 |
