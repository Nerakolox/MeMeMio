...
**状态**：`planning`——契约已写进 SPEC（`proposed`），**两端都还没开工**。两端确认契约后转 `in_progress`。

**性质**：**跨端**——新增接口、新增数据表、改 SPEC 四处；web 需要新的表单卡片（类型仍从 Hono RPC 来，没有生成步骤）。
...
`§9.9` 要求约束写在输入框旁边。初稿如下，**web 执行者不要自行改写或删减**，要改回总管：
...
| 位置 | 现状 / 动作 |
|---|---|
| `spec/03-auth-permission.md` §3.3 | 权限矩阵 +1 行「配置运行参数（并发上限）」——**已改** |
| `spec/05-data-models.md` §5.6 | 新增，含表结构、上下限、可空语义、每进程警告、交叉约束——**已改** |
| `spec/06-endpoints.md` §6.5.5 | 新增，含两个端点、必填整组、生效语义的三种时机表、不做恢复默认/状态端点——**已改** |
| `spec/09-decisions.md` §9.26 | 新增，含划线判据、为什么是 DB 不是 env、每进程偏差、什么情况下推翻——**已改** |
| `api/src/image/constants.ts` 文件头 | 立场被收窄（交 api 执行者改注释） |
| `api/agents/rules/image-pipeline.md` §3 或 §8 | 若其中引用了「是常量，不是环境变量」，同步收窄；**只改被 §9.26 划到运行期的那几个** |
| `web/agents/rules/settings-ux.md` §2 | 分段表 +1 行（交 web 执行者改） |

SPEC 条目状态均为 `proposed`。按 [§8.1](../spec/08-collaboration.md)：两端确认后转 `accepted`，**那时才开始写代码**。
