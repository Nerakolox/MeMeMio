...
/**
 * 单个文件的原始帧数上限。超出报 FILE_TOO_LARGE。
 *
 * **为什么必须有：** 抽帧的成本是 `O(帧数)` 次解码——ffmpeg 那一次是线性的一整趟，
 * 之后**每一帧**还要过一次 sharp 算 pHash（`computeFramePhash`，帧间去重）。
 * 而这条管线的产出上限是 `MAX_FRAMES`（10）张。帧数不封顶的话，一个几千帧的 GIF
 * 会在这里花掉几十秒 CPU，表现是「导入卡住」或者打标任务超时落成 `needs_manual`
 * （5 次重试都是同一个结果）——**失败信息里没有原因**。
 *
 * 取 1000 的依据：**它是产出上限的 100 倍。** 实测 989 张真实库里去重后的状态数
 * 最大 81 帧（`frames.ts` 有记），原始帧数还要高一个量级，1000 之上再多的帧
 * 只会被去重和采样丢掉，付的是纯成本。这不是一个「待校准的起点」，
 * 而是「远高于任何正常文件」的护栏：真有人导入 1000+ 帧的文件时，拿到的是一条
 * **写明帧数的立即失败**，而不是十几秒后一条语焉不详的超时。
 *
 * ⚠️ **不要改成「只取前 N 帧」来绕过拒收。** 采样策略是**偏后段**的
 *    （`frames.ts` 的 `pickFrames`：文字往往到结尾几帧才出现），只处理前 N 帧等于
 *    把最复杂的那些文件系统性地标错，而且**不报错**——正是这个模块要消灭的那类 bug。
 */
export const MAX_RAW_FRAMES = 1000
