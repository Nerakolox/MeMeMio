 ...
 *
 * ⚠️ **改写是原查询的补充，不是替代。** 向量路编码的是「原查询 + 改写」拼在一起
 *   （见 `services/search.ts` 的 `embedInput`）。只编码改写的代价是：改写丢掉的东西
 *   就永久丢掉了，而它最爱丢的正是否定和语气——「不要真人」被改成「一个真人在摆手」，
 *   向量路会兴高采烈地召回一整屏用户明说不要的图，没有任何地方会报错。
 */
...
const SYSTEM_PROMPT = [
  ...
  '',
  '必须原样保留的三样东西：',
  '1. 否定。「不要真人」改写后仍然要是「不是真人」，不能变成「真人」。',
  '2. 语气。敷衍、阴阳怪气、撒娇是三种不同的说法，改写不能把它们抹成中性。',
  '3. 交流用途。用户说「表面同意」，写的就是表面同意，不要写成「同意」。',
  '',
  '不要凭空添加原句里没有的人物、动物、道具或场合——',
  '用户没说猫，改写里就不该出现猫。编出来的具体物件会把检索带到完全无关的图上。',
  '',
