...
**（2026-09-24 更正一处措辞）** 上面那句原本写着「它的表现和『这台浏览器不支持剪贴板写入』完全一样」，**现在只对了一半**：取图失败会明说「取不到原图（可能是 R2 的 CORS 没放行 GET）」并给出一个可点的链接，和权限被拒区分得开（改动见[浏览页图片操作](../joint-tasks/2026-09-19-browse-meme-actions.md)）。**本节的排查顺序不变**，改的是结论那半句——原来那种「两种失败长得一模一样」的分不清，正是它作为排查手册最不该有的性质。
