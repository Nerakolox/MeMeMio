# Mememio 跨端规范

文档基线：**SPEC 0.1.0-draft**。维护者：总管。

**当前没有任何条目为 `stable`**——`stable` 要求已通过验证并进入基线，而本项目还没有一次真实部署与联合验收。设计已对齐的条目为 `accepted`（两端已实现的条目也在这一档），依赖外部探测结果的条目为 `proposed`。状态定义见 [§0.3](00-overview.md)；**标记记的是契约是否对齐，不记实现进度**，口径见 [§9.28](09-decisions.md)。

| § | 文件 | 内容 |
|---|---|---|
| 0 | [00-overview.md](00-overview.md) | 范围、主源、状态与基线 |
| 1 | [01-http.md](01-http.md) | URL、响应信封、时间、分页与 SSE |
| 2 | [02-errors.md](02-errors.md) | 错误结构、错误码与消费行为 |
| 3 | [03-auth-permission.md](03-auth-permission.md) | 会话、角色、权限矩阵与软删边界 |
| 4 | [04-vocabulary.md](04-vocabulary.md) | 固定标签词表的规则与变更流程 |
| 5 | [05-data-models.md](05-data-models.md) | User、Meme、Favorite、AI 配置 |
| 6 | [06-endpoints.md](06-endpoints.md) | 业务 API 及输入输出 |
| 7 | [07-naming.md](07-naming.md) | 通信、数据库与代码命名 |
| 8 | [08-collaboration.md](08-collaboration.md) | 契约变更、任务与归档 |
| 9 | [09-decisions.md](09-decisions.md) | 设计决策与取舍理由 |

## 按任务读取

| 任务 | 必读 |
|---|---|
| 所有接口改动 | §0、§1、§2、§8 与对应模型 / 端点 |
| 登录、邀请与权限 | §3、§6.1；涉及 admin 另读 §3.2 |
| 导入与去重 | §1.4、§5.2、§6.2、§9.7 |
| 打标与词表 | §4、§5.2.3、§6.5、§9.3、§9.5 |
| 打标进度与待处理列表 | §5.2.3、§6.3.2、§6.6 |
| 搜索与排序 | §5.2.4、§6.3、§9.10 |
| 复制 / 分享 / 发送 | §5.2.2、§9.2 |
| 模型配置与测试连接 | §5.3、§6.5、§9.3、§9.7；动 embedding 配置或重建索引另读 §9.6 |
| 删除、编辑与收藏 | §3.3、§3.4、§4.5、§5.4、§6.4（含 §6.4.1 / §6.4.2）、§9.1、§9.19 |

引用使用文件名与稳定章节号。本文只作路由，不能只看关键词表推断契约细节。
