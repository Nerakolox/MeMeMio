...
> **状态：`accepted`**（2026-09-26，[检索与筛选合流](../_archive/joint-tasks/2026-09-26-检索筛选合一.md)）。本节把检索与浏览合流成一个端点：`GET /memes` 吸收 `q`，有 `q` 时做三路融合检索、无 `q` 时按条件浏览，**两者都可翻页**；`GET /search` 冻结为兼容入口。两端已于同日读过并确认，理由见 [§9.29](09-decisions.md)。
...
| GET | `/search` | 兼容入口，≡ `GET /memes?q=` 的冻结子集，见 [§6.3.3](#633-search冻结的兼容入口) |
| GET | `/memes/{id}` | 单条 |
...
`q` 去掉首尾空白后必须非空才走本节。**空串或只有空白按「无 `q`」处理**（走 [§6.3.2](#632-浏览)），不返回错误——用户清空搜索框是常规操作，不是非法请求。（`GET /search` 那边仍然「缺 `q` 就报错」，理由见 [§6.3.3](#633-search冻结的兼容入口)：它的语义是「一次检索」，没有查询词就没有请求的对象。）
...
### §6.3.3 `/search`：冻结的兼容入口

```
GET /search?q=今天真的不想上班&limit=50
```

**它 ≡ `GET /memes?q=` 的一个子集，同一个实现，不再演进。** 只认 `q` 与 `limit` 两个参数，其余参数（含七个筛选维度和 `cursor`）一律 `VALIDATION_FAILED`——[§0.4](00-overview.md) 规定请求里的未知字段返回 `VALIDATION_FAILED`，静默忽略会让一个叫 `/search` 的地址悄悄变成浏览。`q` 必填，去空白后为空同样报错（与 §6.3.1 的处理相反，理由见那节）。响应结构与 `GET /memes?q=` 完全一致，但 `nextCursor` 恒为 `null`——它不分页。

**`limit` 缺省时是 50，不是 [§1.3](01-http.md) 的 40——这是冻结的一部分。** 首页那条路径不传 `limit`，默认值由服务端说了算；把它对齐到 40 等于悄悄改掉首页看到的结果条数，而首页本轮不动（[检索与筛选合流](../_archive/joint-tasks/2026-09-26-检索筛选合一.md) 裁定 3）。所以：显式给 `limit` 就照给（上限仍 100），不给按 **50**；`GET /memes` 不给则按 §1.3 的 **40**。**两处默认值不同是有意的**，首页改版、这个入口删掉之后一起消失。

**为什么留着它：** 首页那条路径目前走的就是它，而首页的去向还没定（[页面地图](../joint-tasks/2026-09-25-web-page-map.md)）；它同时也给调试和将来的评测留了一个「只要查询词」的入口。**新能力一律只加在 `GET /memes` 上**，这里冻结；首页改版之后再谈删除。
