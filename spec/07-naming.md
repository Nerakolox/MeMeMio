...
**`bigint` 列在 JSON 里是十进制字符串，不是 `number`。** 涉及 `size_bytes`、`storage_quota_bytes` 等列（`phash` 不外露）。原因是 JS `number` 放不下 64 位整数，驱动层原样给出字符串，序列化层不做有损转换。前端要参与算术就显式 `Number()` / `BigInt()`，**不要假设它是数字**——手写接口类型时这一处已经错过两次（搜索任务、导入任务），所以 `web` 的响应类型必须从 `api` 派生。
