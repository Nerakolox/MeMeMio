...
**状态**：`done`（2026-09-24 web 端回填：12 条全部落地，真 Chrome 两档 **65/65 断言过**，
`typecheck` + `build` 过；iOS 真机未测那部分由总管裁定转入「真机待测」一批，见 §5）｜ **性质**：web 单端 ｜ 开于 2026-09-24

契约不变。第 2 条用的是 SPEC §1.4 早就写明的做法：「客户端不能把 SSE 当作唯一的结果来源」，要用 `GET /imports/{batchId}` 补齐。所以**不需要 api 配合**。

来源是 2026-09-24 的只读审查。标 ✔ 的条目总管已核实；其余先复核，查无此事的记「不成立」。401 统一跳转、`/auth/me` 网络失败、`/ui` 进生产包这三条在[会话与访问收口](../../joint-tasks/2026-09-24-auth-access-hardening.md)的 web 端，本任务不重复。
...
**转 `done`。** 12 条落地、65/65 断言、`typecheck` + `build` 干净，本端验收已经充分——这十二条的共同特征是「改之前是静默的、改之后有断言钉着」，而不是「效果需要肉眼判断」，所以没有留下需要真机才能关闭的疑问。

**iOS 真机那两条（第 3、4 条）怎么办。** 执行者把「是否可关闭」交回来是对的，裁定如下：

- **不在本任务里挂账。** 把一条已完成的任务压在「等真机」上，结果是**它连同另外五六条一起烂在 `in_progress`**（[首次部署](../../joint-tasks/2026-09-24-first-deploy.md) 就是为这个死锁开的）。真机是**跨任务的外部条件**，不是这一条的欠账。
- **改挂到任务板的「待测清单：真机与部署」**，与[浏览页图片操作](2026-09-19-browse-meme-actions.md)的「手机 → 分享」、[首页随机图墙](2026-09-19-home-random-grid.md)的自动聚焦并成一批，一次上机全部过掉。那里已经写明要看的正是分歧点：Safari 的用户激活过期规则、`canShare` 对 `image/gif` 的答复、分享面板取消时的返回值。
- **代价说清楚**：从今天起到真机跑完之前，**`navigator.share({files})` 这条路径一次都没有真的跑过**。它只在桌面 Chrome 上验过替代形态（预取 blob、点击时用户激活仍在、用的是预取那一份）。「照规则写的」不等于「验过的」，这句话留在板子上。
- `web-interaction-fixes` 自己的范围里**没有**别的未测项：桌面 Chrome 已验，Firefox / Safari 桌面未跑属既有缺口，不是本任务引入。

**顺带记账（不改状态）**：验收过程里抓到的两个额外缺陷已修——浏览页失败后每秒几十次的请求长龙（`failedCursorRef`）、以及量法本身的两处假断言（脚本 `element.click()` 不产生用户激活、替身直传地址写成相对路径落到 vite 上）。前者**不在原清单里**，是这一轮最有价值的一条；后者记下来是因为不修的话第 3 条的断言是假的——**这类「量法本身错了」比缺陷更值得记**。

**仍未做的三件**（执行者已在上面写明，总管确认不需要在本任务里做）：`lib/api.ts` 手写的 `User` / `InviteCode` / `AdminUser`、`cn` 的三种导入写法、收藏乐观更新的三份重复。前两件属重构、第三件要在 [浏览页图片操作](2026-09-19-browse-meme-actions.md) 收口时一起看。
